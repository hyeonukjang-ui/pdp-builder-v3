 ...
 *   2. rawData 경량화 (불필요 필드 제거 → input 토큰 절감)
 *   3. 블록을 2그룹으로 나눠 병렬 API 호출 → 시간 ~50% 단축
 *   4. 결과 합산하여 introBlocks 배열 반환
 */
...
async function extractTextFromImages(imageUrls) {
  ...
  try {
    ...
    const message = await client.messages.create({
      model: 'claude-haiku-4-5-20251001', // 빠른 모델로 텍스트 추출
      max_tokens: 2000,
      messages: [{
        role: 'user',
        content: [
          ...
          {
            type: 'text',
            text: `위 이미지들은 여행 상품 상세페이지에 삽입된 이미지야.
이미지 안에 있는 텍스트 정보를 모두 추출해줘.
특히 다음 정보가 있으면 반드시 포함:
- 투어 코스/일정
- 포함/불포함 사항
- 집합 장소/시간
- 이용 방법
...
텍스트만 간결하게 출력해. 이미지 설명은 불필요.
텍스트가 없는 풍경/음식 사진은 "풍경 이미지" 등 한줄로만.`,
          },
        ],
      }],
    });
    ...
    console.warn('[intro-gen] Vision 추출 실패:', err.message);
    return '';
  }
}

// ─── rawData 경량화 ─────────────────────────────────────────
// AI에 불필요한 필드를 제거하여 input 토큰을 줄인다.

function slimRawData(rawData) {
  const slim = { ...rawData };
  // HTML blob 제거
  delete slim.descriptionHtml;
  delete slim.rawHtml;
  delete slim.htmlContent;
  // 중복/불필요 메타
  delete slim.seoMeta;
  delete slim.jsonLd;
  delete slim.breadcrumb;
  delete slim.relatedProducts;
  delete slim.similarProducts;
  // 이미지는 URL만 유지 (alt, width, height 등 제거)
  if (slim.images) {
    slim.images = slim.images.slice(0, 16).map((img) => ({ url: img.url }));
  }
  // 옵션 상세는 이름/가격만
  if (slim.options && Array.isArray(slim.options)) {
    slim.options = slim.options.map((opt) => ({
      name: opt.name || opt.title,
      price: opt.price || opt.salePrice,
      description: opt.description,
    }));
  }
  // 리뷰 본문은 제거, 요약 통계만
  if (slim.reviews && Array.isArray(slim.reviews)) {
    slim.reviewSummary = {
      count: slim.reviews.length,
      avgRating: slim.rating || slim.averageRating,
    };
    delete slim.reviews;
  }
  return slim;
...
export async function generateIntroBlocks({ rawData, category, depth2, recipe: customRecipe, speed = 'normal' }) {
  ...
  const liteData = slimRawData(rawData);

  const modelMap = {
    fast: 'claude-haiku-4-5-20251001',
    normal: process.env.AI_MODEL_COPY || 'claude-sonnet-4-20250514',
    opus: 'claude-opus-4-20250514',
  };
  const model = modelMap[speed] || modelMap.normal;

  // ── recipe 분할 — recipe가 길수록 더 잘게 쪼개 그룹당 토큰을 줄이고 prompt cache hit을 노린다
  // (그룹 1개는 cache miss, 나머지는 hit → opus 같이 느린 모델에서 효과 큼)
  let groups;
  if (recipe.length <= 6) groups = [recipe];
  else if (recipe.length <= 10) {
    const mid = Math.ceil(recipe.length / 2);
    groups = [recipe.slice(0, mid), recipe.slice(mid)];
  } else {
    const size = Math.ceil(recipe.length / 4);
    groups = [
      recipe.slice(0, size),
      recipe.slice(size, size * 2),
      recipe.slice(size * 2, size * 3),
      recipe.slice(size * 3),
    ].filter(g => g.length);
  }
  ...
  // Vision과 AI 호출을 동시에 시작 (이전엔 vision await 후 AI → 누적 직렬).
  // Vision은 보조 보강용이라 AI 호출이 vision 결과 없이 진행돼도 무방.
  const descImages = (rawData.descriptionImages || []).slice(0, 5);
  const visionPromise = descImages.length
    ? extractTextFromImages(descImages).catch(() => '')
    : Promise.resolve('');

  const aiPromise = Promise.all(groups.map(g => callAI(liteData, category, g, model)));

  const [aiResults, visionText] = await Promise.all([aiPromise, visionPromise]);
  if (visionText) liteData._visionExtracted = visionText; // 후속 호출에서 활용 가능 (현재는 미사용)

  const allBlocks = aiResults.flatMap((r, i) => parseIntroResponse(r.text, groups[i]));
  const totalInput = aiResults.reduce((s, r) => s + r.inputTokens, 0);
  const totalOutput = aiResults.reduce((s, r) => s + r.outputTokens, 0);
  ...
  if (groups.length === 1) {
    return {
      success: true,
      category,
      introData: { category, introBlocks: allBlocks },
      usage: {
...
async function callAI(liteData, category, recipeGroup, model) {
  const { system, user } = buildIntroPromptSplit(liteData, category, recipeGroup);
  const maxTokens = Math.min(4096, 512 + recipeGroup.length * 350);
...
// system/user 분리 버전 — prompt caching용
function buildIntroPromptSplit(rawData, category, recipe) {
  const categoryTone = getCategoryTone(category);
  const images = (rawData.images || []).slice(0, 16);
  const imageIndex = images.map((img, i) => `[${i}] ${img.url}`).join('\n');
  ...
  // system: 고정 지시 + rawData (두 병렬 호출이 공유 → cache hit)
  const system = `너는 마이리얼트립의 수석 카피라이터야. 여행 상품 PDP의 "상품소개" 탭에 들어갈 세로 스크롤 페이지 카피를 생성해.

## 브랜드 보이스
"여행자의 현명한 친구" — 솔직하고, 구체적이고, 편안하고, 행동지향적. ~해요체 기본.

## 카피 원칙
1. 구체적 숫자 우선 2. 행동 동사 포함 3. 한 문장 최대 50자 4. 구어체 5. 팩트만 사용

## 절대 금지
최상급(최고/완벽/압도적), 가짜 긴급성, 번역체, 과장, 허위정보, 본문 이모지, 뻔한 문장
...
## 이미지 목록
${imageIndex}${rawData._visionExtracted ? `

## 기존 상세페이지 이미지에서 추출한 텍스트
아래는 기존 상세페이지에 삽입된 이미지를 분석하여 추출한 텍스트야.
이 정보도 카피 생성 시 적극 참고해. 특히 투어 코스, 일정, 포함/불포함, 집합 장소 등 구체적 정보가 있으면 반드시 활용해.
\`\`\`
${rawData._visionExtracted}
\`\`\`` : ''}`;

  // user: 블록 스키마 + 출력 지시 (그룹마다 다름)
  const user = `## 생성할 블록 (${blockEntries.length}개)
모든 블록 필수. 빈 객체 반환 금지.
...
## 상품 이미지 목록 (인덱스로 참조)
아래 이미지 중 각 섹션(카드 그리드, 프로그램 등)의 내용에 **가장 적합한 이미지 인덱스**를 선택해서 imageIndex 필드에 넣어줘.
상품 설명과 이미지를 비교해서 관련도가 높은 것을 매칭해. 같은 이미지를 중복 사용하지 마.
\`\`\`
${imageIndex}
\`\`\`
...
- paragraphs: 2~3개 문단. 이 상품의 차별점/핵심 가치.` : ''}${entry.label === '포함·불포함' ? `**시트 규칙: 상품에 포함/불포함된 항목을 2열 카드로 나열.**
- title: "포함·불포함"
- **반드시 includes, excludes 배열 사용 (paragraphs 금지)**
- includes: 포함 항목 문자열 배열 (예: ["전문 가이드", "생수", "입장료"])
- excludes: 불포함 항목 문자열 배열 (예: ["식비", "개인 경비"])
- 원본 데이터에 있는 것만. 없으면 합리적으로 추정.
- 출력 예시: { "title": "포함·불포함", "includes": ["...","..."], "excludes": ["...","..."] }` : ''}${entry.label === '집합/픽업' ? `**시트 규칙: 집합 장소, 픽업 정보를 안내.**
- title: "집합/픽업 안내"
...
- title: "위치·교통"
- paragraphs: 주소, 교통편, 소요시간 등. 원본 데이터에 있는 것만. 없으면 합리적으로 추정하여 반드시 생성.` : ''}${entry.label === '티켓 옵션 비교표' ? `**시트 규칙: 표로 삽입. 옵션명, 이용대상, 특징으로 노출. paragraphs 사용 금지.**
- title: "티켓 옵션 비교"
- table: 배열. 각각 { "name": "옵션명", "target": "이용대상", "feature": "특징" }. 2~5개.
- paragraphs 대신 **반드시 table 배열 사용**.` : ''}
...
JSON: { "title": "자주 묻는 질문", "faq": [{ "q": "질문", "a": "답변" }, { "q": "질문2", "a": "답변2" }] }` : ''}${entry.label === '포함·불포함' ? `**포함·불포함은 반드시 includes, excludes 배열 사용. paragraphs 사용 금지.**
JSON: { "title": "포함·불포함", "includes": ["포함1", "포함2"], "excludes": ["불포함1", "불포함2"] }` : ''}
\`\`\`json
${entry.label === 'FAQ' ? '{ "title": "자주 묻는 질문", "faq": [{ "q": "질문", "a": "답변" }] }' : entry.label === '포함·불포함' ? '{ "title": "포함·불포함", "includes": ["전문 가이드", "생수"], "excludes": ["식비", "입장료"] }' : entry.label === '티켓 옵션 비교표' ? '{ "title": "티켓 옵션 비교", "table": [{ "name": "성인", "target": "만 18세 이상", "feature": "모든 전시 입장" }] }' : '{ "title": "string", "paragraphs": ["string", "string"], "background": "white" }'}
\`\`\``,
...
\`\`\``,
    ...
    // intro-course 스키마 삭제: 레시피에서 사용되지 않으며 intro-blocks/index.js에도 미등록
...
**guides 배열** 구조로 생성. 가이드가 여러 명이면 배열에 모두 포함.
규칙:
- guides: 배열. 각 가이드 객체:
  - name: 이름 (데이터에 있는 그대로)
  ...
  - image: null (이미지는 코드에서 자동 배정)
- **데이터에 가이드 정보가 없으면 합리적으로 추정하여 반드시 생성** — 만들어내기 금지
\`\`\`json
{ "guides": [{ "name": "string", "role": "string", "experience": "string", "badges": ["string"], "introduction": "string", "image": null }] }
\`\`\``,
...
렌더러 레이아웃: 등급뱃지 + 호텔명 + 영문명 + 하이라이트(아이콘+텍스트) + 메타(아이콘+텍스트) + 1+4 이미지 그리드.
규칙:
- name: 정확한 호텔명 (데이터에 있는 그대로)
- nameEn: 호텔 영문명. 데이터에 있으면 그대로, 없으면 생략.
- grade: 등급 텍스트. "5성급", "디럭스" 등. (starRating 대신 사용)
- highlights: 호텔 하이라이트 배열 2~4개. 각각 { "icon": "이모지", "text": "설명" }. 핵심 편의시설/특장점.
- meta: 위치·교통 정보 배열 1~3개. 각각 { "icon": "이모지", "text": "설명" }. 예: { "icon": "📍", "text": "시내 중심, 지하철 5분" }
- images: 호텔 이미지 배열. 최대 5개. 각각 { "url": "string" } 또는 빈 배열 []. 이미지는 코드에서 자동 배정.
- **숙소 데이터가 없으면 반드시 합리적으로 추정하여 반드시 생성** — 호텔 정보 만들어내기 금지
\`\`\`json
{ "name": "호텔명", "nameEn": "Hotel Name", "grade": "5성급", "highlights": [{ "icon": "🏊", "text": "인피니티 풀" }, { "icon": "🍳", "text": "조식 포함" }], "meta": [{ "icon": "📍", "text": "시내 중심, 지하철 5분" }], "images": [] }
\`\`\``,
...
- items: 최대 3개, 각각:
  ...
  - subtitle: **2~3줄 설명 (60~100자). 그곳에서 뭘 구경하고, 뭘 체험하고, 뭘 즐길 수 있는지 구체적으로.**
  - imageIndex: **상품 이미지 목록에서 이 항목에 가장 적합한 이미지 인덱스 번호.** 상품 설명과 이미지 순서를 비교해서 관련도가 높은 것을 선택. 같은 인덱스 중복 사용 금지.
- **데이터에 있는 장소만 사용.**
\`\`\`json
{ "title": "주요 명소", "items": [{ "title": "장소명", "subtitle": "설명", "imageIndex": 2 }] }
\`\`\``,
  };
...
function buildFallbackData(blockType) {
  const fallbacks = {
    ...
    'intro-schedule': { title: '일정', days: [{ label: 'D1', title: '일정 1', summary: '' }] },
    'intro-provider': { guides: [{ name: '가이드', role: '', introduction: '', image: null }] },
    'intro-comparison': { title: '옵션 비교', items: [{ name: '옵션', price: 0, included: true }] },
    'intro-program': { title: '프로그램', items: [{ label: '', title: '항목 1', description: '', images: [] }] },
    'intro-card-grid': { title: '주요 명소', items: [{ title: '명소 1', subtitle: '', image: {} }] },
    'intro-accommodation': { name: '숙소', nameEn: '', grade: '', highlights: [], meta: [], images: [] },
    'intro-stat': { items: [{ value: '0', unit: '', label: '수치' }], background: 'gray' },
    'intro-safety': { title: '안전 장비', equipment: [], description: '' },
  };
