...
- title: "위치·교통"
- paragraphs: 주소, 교통편, 소요시간 등. 원본 데이터에 있는 것만. 없으면 빈 객체 {} 반환.` : ''}${entry.label === '티켓 옵션 비교표' ? `**시트 규칙: 표로 삽입. 옵션명, 이용대상, 특징으로 노출.**
- title: "티켓 옵션 비교"
- paragraphs: 옵션별 이름/이용대상/특징을 표 형태로 정리. 원본 데이터의 옵션 정보에서 추출. 없으면 빈 객체 {} 반환.` : ''}
...
**시트 규칙 [${cat}]: ${cat === 'TOUR' ? 'AI (일정 데이터 있을 때). 투어 일정 타임라인.' : cat === 'TICKET' ? '직접매핑 (공연 전용). 공연 시간표.' : '일정 타임라인.'}**
규칙:
...
**시트 규칙 [${cat}]: CTA 버튼 노출, 링크 삽입 기능. 상품추천 카드 (가로 스크롤). ${cat === 'TOUR' ? '같은 도시/유사 투어 추천.' : cat === 'TICKET' ? '같은 도시/유사 티켓 추천.' : '유사 상품 추천.'}**
규칙:
...
**시트 규칙 [${cat}]: ${cat === 'TOUR' ? '투어 코스/명소. 정보 있으면 불러와서 텍스트 생성. 이미지 + AI.' : cat === 'TICKET' ? '어트랙션/포함시설. 직접매핑 + AI. 입장권/시티패스에서 사용.' : cat === 'SNAPS' ? '업소·매장 목록.' : '카드 그리드.'}**
2열 이미지 카드 그리드.
규칙:
