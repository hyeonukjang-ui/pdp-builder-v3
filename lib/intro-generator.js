...
export async function generateIntroBlocks({ rawData, category, depth2, recipe: customRecipe, speed = 'normal' }) {
  ...
  // 기존 상세페이지 이미지에서 텍스트 추출 (Vision)
  const descImages = (rawData.descriptionImages || []).slice(0, 5);
  let visionText = '';
  if (descImages.length) {
    console.log(`[intro-gen] 상세 이미지 ${descImages.length}장 Vision 분석 시작`);
    visionText = await extractTextFromImages(descImages);
    if (visionText) {
      liteData._visionExtracted = visionText;
      console.log(`[intro-gen] Vision 추출 완료 (${visionText.length}자)`);
    }
  }
  ...
  if (recipe.length <= 6) {
    return singleCall(liteData, category, recipe, model);
  }

  const mid = Math.ceil(recipe.length / 2);
  const groupA = recipe.slice(0, mid);
  const groupB = recipe.slice(mid);

  const startTime = Date.now();
  const [resultA, resultB] = await Promise.all([
    callAI(liteData, category, groupA, model),
    callAI(liteData, category, groupB, model),
  ]);

  const allBlocks = [
    ...parseIntroResponse(resultA.text, groupA),
    ...parseIntroResponse(resultB.text, groupB),
  ];

  const totalInput = resultA.inputTokens + resultB.inputTokens;
  const totalOutput = resultA.outputTokens + resultB.outputTokens;
...
async function singleCall(liteData, category, recipe, model) {
  const { system, user } = buildIntroPromptSplit(liteData, category, recipe);
  const startTime = Date.now();
