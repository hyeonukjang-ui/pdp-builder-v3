...
- title: "취소 및 환불 안내"
- paragraphs: 취소 기한, 환불 조건 등. 원본 데이터에 있는 것만. 없으면 합리적으로 추정하여 반드시 생성.` : ''}${entry.label === 'FAQ' ? `**시트 규칙: 자주 묻는 질문 3~5개.**
- title: "자주 묻는 질문"
...
- 데이터에 관련 상품 정보가 없으면 합리적으로 추정하여 반드시 생성.
\`\`\`json
...
- **데이터에 있는 장소/옵션만 사용.** 없으면 합리적으로 추정하여 반드시 생성.
\`\`\`json
