...
- title: "취소 및 환불 안내"
- paragraphs: 취소 기한, 환불 조건 등. 원본 데이터에 있는 것만. 없으면 빈 객체 {} 반환.` : ''}${entry.label === 'FAQ' ? `**시트 규칙: 자주 묻는 질문 3~5개.**
- title: "자주 묻는 질문"
- paragraphs: Q&A 형태로 3~5개. 상품 데이터에서 예상 가능한 질문만.` : ''}${entry.label === '공지/주의사항' ? `**시트 규칙: 필독 공지, 주의사항.**
- title: "꼭 알아두세요"
- paragraphs: 주의사항/공지 bullet 정리. 원본 데이터에 있는 것만.` : ''}${entry.label === '노선·구간' ? `**시트 규칙: 교통 노선, 구간 정보.**
- title: "노선·구간 안내"
- paragraphs: 노선/구간/정류장 정보. 원본 데이터에 있는 것만. 없으면 빈 객체 {} 반환.` : ''}${entry.label === '위치·교통' ? `**시트 규칙: 위치, 교통 안내.**
- title: "위치·교통"
- paragraphs: 주소, 교통편, 소요시간 등. 원본 데이터에 있는 것만. 없으면 빈 객체 {} 반환.` : ''}
- background: "white" 또는 "gray"
- 한 문장 최대 50자. 데이터에 해당 정보가 없으면 빈 객체 {} 반환.
\`\`\`json
...
**시트 규칙: 아이콘 + 텍스트 조합으로 핵심 매력 2열 그리드 배치.**
규칙:
- title: "하이라이트" 또는 맞춤 제목, 최대 15자
- items: 4~6개. 각각:
  - icon: 적절한 이모지 1개
  - label: 구체적 대상, 최대 10자. 숫자 포함 권장. "다양한 체험" (X) → "7가지 어트랙션" (O)
...
**시트 규칙: 투어/상품 주제에 맞는 추천 대상 노출.**
규칙:
- tag: "이런 분께 추천해요" 등 태그 텍스트
- title: 추천 대상 핵심. "가족 여행자", "첫 방문자" 등 구체적. 최대 20자.
- description: 왜 추천하는지 구체적으로. 최대 80자.
\`\`\`json
{ "tag": "이런 분께 추천해요", "title": "string", "description": "string" }
\`\`\``,
...
**시트 규칙: 직접매핑 + AI + 사진 불러와서 넣기. 48px 원형 프로필.**
규칙:
- name: 이름 (데이터에 있는 그대로)
...
- **데이터에 가이드 정보가 없으면 빈 객체 {} 반환** — 만들어내기 금지
\`\`\`json
...
**시트 규칙: 이용방법 데이터 불러오기. 예약확정→이용 절차.**
규칙:
- title: "이용 방법", 최대 15자
- steps: 3~5개, 각각:
  - step: 번호
...
**시트 규칙: CTA 버튼 노출, 링크 삽입 기능. 상품추천 카드 (가로 스크롤).**
규칙:
- title: "이런 상품은 어때요?" (기본값)
- items: 2~4개 추천 상품 카드. 각각:
  - title: 상품명
  - price: 가격 (예: "45,000원~")
  - desc: 한줄 설명
  - url: 상품 URL (빈 문자열로)
  - image: null (사용자가 직접 추가)
- 데이터에 관련 상품 정보가 없으면 빈 객체 {} 반환.
\`\`\`json
{ "title": "이런 상품은 어때요?", "items": [{ "title": "추천 상품명", "price": "45,000원~", "desc": "한줄 설명", "url": "", "image": null }] }
\`\`\``,
...
\`\`\``,
    ...
    'intro-card-grid': `### [${entry.index}] intro-card-grid (${entry.label || '투어 코스/명소'}) — ${entry.required ? '필수' : '선택'}
**시트 규칙: 정보 있으면 불러와서 텍스트 생성. 이미지 + AI.**
2열 이미지 카드 그리드.
규칙:
- title: "${entry.label || '주요 명소'}" 등 섹션 제목. 최대 15자.
- items: 2~6개 카드, 각각:
  - title: 장소명/어트랙션명. 최대 15자. 데이터에 있는 이름 그대로.
  - subtitle: 부가 정보. 최대 15자. (선택)
  - tag: 태그. 최대 8자. (선택)
  - image: 빈 객체 {} (이미지는 사용자가 직접 추가)
- **데이터에 있는 장소/옵션만 사용.** 없으면 빈 객체 {} 반환.
\`\`\`json
{ "title": "주요 명소", "items": [{ "title": "장소명", "subtitle": "부가 정보", "tag": "인기", "image": {} }] }
\`\`\``,
  };
...
function buildFallbackData(blockType) {
  const fallbacks = {
    ...
    'intro-cta': { title: '이런 상품은 어때요?', items: [{ title: '추천 상품', price: '', desc: '', url: '', image: null }] },
